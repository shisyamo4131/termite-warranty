...
- Build with `npm run build`, then deploy with an explicit target: `npx firebase deploy --project termite-warranty-dev --only firestore,functions,hosting`. Do not create a default Firebase project alias. Development Functions use `asia-northeast1` and the SPA is served from `.output/public` with an `index.html` fallback.
