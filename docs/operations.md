...
- Build the Hosting artifact with `npx nuxt generate`; it produces `.output/public/index.html` for the Nuxt SPA. The approved development deployment route is the GitHub Actions workflow `Deploy Firebase development`, triggered by a push to GitHub's `main` branch or manually through `workflow_dispatch`. It authenticates with the repository secret `FIREBASE_SERVICE_ACCOUNT_TERMITE_WARRANTY_DEV` and deploys only to the explicit `termite-warranty-dev` target: `npx firebase deploy --project termite-warranty-dev --only firestore,functions,hosting --non-interactive --force`. The `--force` option creates Firebase's recommended Artifact Registry cleanup policy when it is absent. Do not create a default Firebase project alias. Development Functions use `asia-northeast1` and the SPA is served from `.output/public` with an `index.html` fallback. The service-account key is stored only as a GitHub Actions secret and must never be added to source control or logs. Before Functions deployment, Firebase predeploy invokes `npm run prepare:functions`; it stages and byte-verifies the shared modules from the sole authored `src/domain` source. The generated `functions/domain/` directory and temporary staging directories are never committed. Run `npm run check:functions-domain` for a read-only missing/extra/content-drift check.
...
For this initial mixed implementation, run each selected verification command separately and record its exit status: `npm test`, `npm run typecheck`, `npm run build`, `npm run test:rules`, `node scripts/check-functions-syntax.mjs`, `node --check scripts/seed-emulator.mjs`, and `./scripts/check-governance.ps1`. Functions syntax and Rules/Callable Emulator paths prepare the shared Functions domain automatically. For packaging diagnostics, run `npm run prepare:functions` and then `npm run check:functions-domain` separately. For later work, select only the gates required by every applicable class in the policy below.
...
- Gate: id=domain-test; command=npm test; stages=[iteration,targeted,completion]; includes=[]; invalidatedBy=[src/domain/**\, functions/local-runtime.js\, Functions domain preparation helpers/tests\, test domain/runtime files\, package.json\, package-lock.json\, or the Node.js runtime changes]; evidenceDestination=task completion report
