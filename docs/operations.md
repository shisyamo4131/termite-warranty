...
- Implemented locally: warranty calculations and N-Gram generation, a Nuxt/Vuetify SPA, Firebase Authentication emulator login, Firestore emulator persistence and rules, synthetic seed data, callable Admin SDK atomic case/first-warranty registration and applied-warranty management, required case application/handover dates, button-triggered create/edit dialogs for four masters and cases, dedicated master and case detail routes with list-return/not-found handling, construction-company linked-property detail display, construction-company and homeowner address/contact fields, four-master quick creation that preserves the case draft, property-defaulted and independently editable case homeowner/company references, non-propagating property-party edits, transactional active-case editing, bounded freshest-20 case/master lists, parent-case warranty list projections, client-side current-window filters, current-master case-list joins, dashboard alert evaluation and Navigation Drawer, direct Rules-governed reversible CRUD for four masters, Clear Sky Vuetify theme styling, and Vuetify date selection.
- Development environment provisioned: Firebase project `termite-warranty-dev`, web app `TermiteWarranty Dev Web`, Hosting site `termite-warranty-dev`, email/password Authentication, and default Firestore in `asia-northeast1` (Tokyo). Hosting and Functions deployment/acceptance remain incomplete until recorded successful deployment and smoke verification.
- Planned but unavailable: branch/account management, postal-code API integration, production Firebase project, production security hardening, monitoring, backup/recovery, and the no-migration cutover procedure. FileMaker data migration is not in scope.
- The local Functions export set is limited to atomic case registration and applied-warranty add/update commands. Staff profile state is read through the Rules-governed live Firestore subscription. Source removal of a function does not authorize remote deletion; inventory and deletion remain separate deployment approvals.
