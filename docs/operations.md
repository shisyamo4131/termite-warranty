...
- Planned but unavailable: branch/account management, postal-code API integration, FileMaker migration, real development/production Firebase projects, Hosting, deployment, production security hardening, monitoring, and backup/recovery.
...
Pre-change local emulator documents without the new required company address or case dates remain readable. A normal edit must fill the missing required fields. Clearing and reseeding the disposable emulator is the simplest way to obtain a fully current synthetic dataset; this is not a production migration or backfill procedure.
