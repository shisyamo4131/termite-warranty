...
- A shared-account construction-company portal is being implemented as a proposal prototype. It does not yet have House Solution acceptance, real email delivery, scheduled renewal generation, repeat-cycle history, or production external-access controls.
- Firebase Emulator Suite remains the local regression environment. The developer-owned `termite-warranty-dev` environment is provisioned and revision `967bd1cc37fb025ee7356274c60a94a550fc8270` is technically deployed with successful smoke evidence; House Solution development-environment acceptance remains incomplete.
