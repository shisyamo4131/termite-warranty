...
| `homeowners/{homeownerId}` | `name: string`, `active: boolean`, `nameSearch.normalized: string`, `nameSearch.one: map<string, true>`, `nameSearch.two: map<string, true>`, `revision: positive integer`, `createdAt: Timestamp`, `updatedAt: Timestamp` |
...
- Pre-change local synthetic company or case documents remain readable. A normal edit must supply the newly required company address or case dates. No production backfill or migration is authorized by this prototype contract.
...
- Construction-company creation and editing enforce the required address parts, preserve optional contact values, and tolerate readable legacy name-only records until they are edited.
