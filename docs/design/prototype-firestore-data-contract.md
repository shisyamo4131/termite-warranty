...
- Approved target divergence: ADR 0016 replaces stale-revision rejection for master changes with last-write-wins. The current prototype behavior above remains implemented until TR-001 changes the callable contract, implementation, and tests.
...
- Four-master create, read, update, inactivate, and reactivate behavior currently preserves IDs, validates references, regenerates search tokens, and rejects stale revisions unchanged. TR-001 must replace only the stale-rejection behavior with the approved last-write-wins rule.
