...
defineOptions({ name: 'HomeOwnerTable', inheritAttrs: false })

const { smAndUp, mdAndUp, lgAndUp } = useDisplay()

const columns: Record<'name' | 'address' | 'telephone' | 'fax' | 'operation', TableHeader> = {
  name: { title: '名称', key: 'name' },
  address: { title: '住所', key: 'address' },
  telephone: { title: '電話番号', key: 'telephone' },
  fax: { title: 'FAX番号', key: 'fax' },
...
const resolvedHeaders = computed<TableHeader[]>(() => {
  ...
  if (lgAndUp.value) result.push(columns.telephone, columns.fax)
